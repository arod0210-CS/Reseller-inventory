(function (global) {
  ...
  function lookupBarcode(barcode, options) {
    ...
    if (options && options.skipUPCitemdb) {
  ...
  async function runScannerLookup(config) {
    ...
    const backendEndpoint = getConfiguredBackendEndpoint(config);
    const backendResult = await lookupAiBackend({
      barcode: barcode,
      imageDataUrl: imageDataUrl,
      backendEndpoint: backendEndpoint
    });
    // When backend is configured it already proxies UPCitemdb — skip direct browser call to avoid WebView CORS blocks
    const lookup = backendResult || await lookupBarcode(barcode, { skipUPCitemdb: !!backendEndpoint });
